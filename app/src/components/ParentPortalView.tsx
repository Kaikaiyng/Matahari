...
function ResultRow({ subject, assessment, score, grade }: { subject: string; assessment: string; score: string; grade: string }) { return <button type="button" className="result-row" disabled><span><strong>{subject}</strong><small>{assessment} · example only</small></span><b>{score}</b><i>{grade}</i><ChevronRight /></button> }
