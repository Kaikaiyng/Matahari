...
export function TeacherPortalView({ teacherName, activeTab, onTabChange, onLogout, staffMode = false }: { teacherName: string; activeTab: string; onTabChange: (tab: string) => void; onLogout: () => void; staffMode?: boolean }) {
  ...
  if (activeTab === 'classes') return <ClassesPage staffMode={staffMode} onAssessments={() => onTabChange('assessments')} onQuizzes={() => onTabChange('quizzes')} onReview={() => onTabChange('review')} />
  ...
  if (activeTab === 'assessments') return <AssessmentPage staffMode={staffMode} />
  if (activeTab === 'quizzes') return <QuizAuthoringPage staffMode={staffMode} />
  return <><TeacherMore teacherName={teacherName} staffMode={staffMode} onAssessments={() => onTabChange('assessments')} onQuizzes={() => onTabChange('quizzes')} onSafety={() => onTabChange('safety')} onLogout={onLogout} /><CommunitySafetyLinks /></>
}
...
function ClassesPage({ staffMode, onAssessments, onQuizzes, onReview }: { staffMode: boolean; onAssessments: () => void; onQuizzes: () => void; onReview: () => void }) {
...
function AssessmentPage({ staffMode = false }: { staffMode?: boolean }) {
  const [assignments, setAssignments] = useState<TeacherAssignment[]>([])
...
function QuizAuthoringPage({ staffMode = false }: { staffMode?: boolean }) {
  ...
  useEffect(()=>{(staffMode ? portalApi.getStaffAssignments() : portalApi.getTeacherAssignments()).then(({data})=>{setAssignments(data);setAssignmentId(data[0]?.id??0)}).catch(()=>setNotice('Unable to load teaching assignments.'))},[staffMode])
