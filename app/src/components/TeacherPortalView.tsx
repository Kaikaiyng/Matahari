    ...
    <div className="portal-viewpager-container">
      ...
      >
        ...
        <div className={`portal-tab-slide ${visibleTab === 'more' ? 'active' : ''} ${isDragging ? 'swiping' : ''}`}>
          <TeacherMore teacherName={teacherName} staffMode={staffMode} canPublishUpdates={canPublishUpdates} onAssessments={() => onTabChange('assessments')} onQuizzes={() => onTabChange('quizzes')} onSafety={() => onTabChange('safety')} onLogout={onLogout} />
        </div>
      </div>
    </div>
    ...
    <div className="record-page">
      ...
      <section className="teacher-quick-links">
        ...
        {canPublishUpdates && <button type="button" onClick={onCreatePost}>
          <MessageCircle />
          <span><strong>School updates</strong><small>Publish for the whole school or selected classes</small></span>
          <ChevronRight />
        </button>}
      </section>
