...
function SafetyLinkList({ links }: { links: SafetyLink[] }) {
  return <div className="about-policy-list-card safety-centre-link-list">{links.map(({ slug, title, description, icon: Icon }) => <a className="about-policy-item safety-centre-link" href={`/legal/${slug}`} key={slug} aria-label={title}><span className="safety-centre-link-icon"><Icon size={20} /></span><span className="about-item-text"><strong>{title}</strong><small>{description}</small></span><ChevronRight size={18} className="about-item-arrow" /></a>)}</div>
}
...
export function CommunitySafetyCentre({ onBack }: { onBack?: () => void }) {
  ...
  return createPortal(<div className={`subpage-slide-overlay ${isExiting ? 'subpage-slide-out' : ''}`} role="region" aria-label="Community Safety Centre Subpage" {...gestureHandlers} style={{ position: 'fixed', inset: 0, zIndex: 99990, background: '#f6f3ee', overflowY: 'auto', ...surfaceStyle }}><div className="subpage-container safety-centre-page">
    ...
    <section className="subpage-content-group"><div className="subpage-section-heading"><b>Safety standards</b></div><SafetyLinkList links={safetyLinks} /></section>
    <section className="subpage-content-group"><div className="subpage-section-heading"><b>Policies &amp; account</b></div><SafetyLinkList links={policyLinks} /></section>
    <section className="subpage-content-group"><div className="subpage-section-heading"><b>School support</b></div><a className="safety-support-card" href="/legal/support" aria-label="Contact Support"><span className="safety-support-icon"><Headphones size={22} /></span><span className="safety-support-copy"><small>NEED HELP?</small><strong>Contact Support</strong><p>Call, WhatsApp or email your school support team.</p></span><ChevronRight size={19} className="about-item-arrow" /></a></section>
