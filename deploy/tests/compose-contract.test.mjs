...
test('database Compose keeps MariaDB private and pinned', async () => {
  ...
  assert.match(compose, /--log-bin=matahari-bin/)
  assert.match(compose, /--binlog-expire-logs-seconds=2678400/)
  assert.match(compose, /name: matahari_database/)
})
...
test('application Compose binds only to loopback and hardens release mounts', async () => {
  ...
  assert.match(compose, /bootstrap_cache:\/var\/www\/matahari\/current\/backend\/bootstrap\/cache/)
})
