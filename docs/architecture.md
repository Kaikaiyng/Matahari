...
The Admin shell owns shared `--admin-card-*` presentation tokens for radius, border, surface depth, hover depth, and inset surfaces. `StatCard`, `DataPanel`, toolbars, Dashboard panels, and established domain-card selectors consume these tokens so Dashboard, Students, Classes, Finance, Attendance, Calendar, Audit/Application Logs, Settings, and moderation share one visual language without rewriting their business components. These tokens are scoped to Admin and do not affect `app/`.
