...
**2026-09-05 product direction:** The active project is Matahari. RYLAY SaaS is deferred. Existing tenant, membership, permission, branding and feature structures remain, while dedicated deployment enforcement is active.
...
The backend resolves an active organization from an explicitly verified request hostname before authentication and business authorization. `TenantContext` contains tenant and surface; active membership then supplies roles and permitted schools. Client-supplied tenant IDs are not authoritative. Each school belongs to one tenant and existing business records continue to inherit tenant ownership through `school_id`.

`TENANCY_MODE=dedicated` requires a valid `TENANCY_DEDICATED_TENANT_SLUG`. Matahari uses `mis`: verified domains belonging to any other tenant return 404, and `/api/v1/platform/*` is unavailable. Missing or invalid tenancy configuration returns 503. The localhost fallback is limited to Laravel's `local` and `testing` environments and resolves only the configured MIS slug. `multi_tenant` remains an explicit future/test mode; it is never selected implicitly.
...
Admin and App run the same Matahari release and use host-resolved `branding` and `features`. A future multi-tenant RYLAY deployment can reuse this core by explicitly selecting `multi_tenant`; tenant-specific frontend copies, permanent customer branches, and tenant-specific authorization behavior remain outside the architecture.

Laravel's `tenant.surface` middleware is the authoritative browser-surface boundary. Admin/tenant-management and legacy finance APIs require `admin`; Community, Teacher, Assessment, Quiz and Portal APIs require `app`; tenant context and session bootstrap/authentication remain shared. A surface mismatch returns 404 and does not replace membership, permission, feature, school or resource authorization. The additional `tenancy.multi` middleware closes platform-level tenant APIs in dedicated mode.
...
The App exposes public `/legal/*` routes backed by a no-session public API. Responses contain effective policy sections, tenant presentation, global safety contacts, and—only for an unambiguous single-active-school tenant—the local school Support contacts. Multi-school tenants do not guess a public school. Responses never include report, reporter, evidence, audit, or internal setting data.
...
- `src/features/settings/`: responsive Admin Settings workspace. It composes school-scoped School Information/App Support APIs with tenant Branding and Attendance Settings APIs, keeps account context read-only, and links access management to Employees.
...
The Settings workspace follows the same MAW-inspired information architecture while retaining Matahari/MIS colours and controls. School Information updates typed `schools` fields; App Support uses a separate one-to-one school record and live contact preview. Both mutations are school-resolved and transactionally audited. Branding changes update the in-memory tenant context after a successful backend save so the current Admin title and CSS variables refresh without a reload. Attendance times and guardian campus-notification switches intentionally share the authoritative Attendance Settings payload; Notifications does not introduce a second settings store or expose planned external destinations.

The Admin shell owns shared `--admin-*` presentation tokens for its 12px primary radius, 16px sibling gap, neutral border and inset surfaces, subtle depth, focus ring, and motion timings. Dashboard is the canonical card reference: shared statistic cards keep text on the left and their soft semantic icon tile on the right. `PersonalAdminPattern.css`, `StatCard`, `DataPanel`, toolbars, custom searchable selects, system date/time pickers, dialogs, and established domain-card selectors apply the same contract to Students, Classes, Parents, Employees, Finance, Attendance, Calendar, Audit/Application Logs, Settings, and moderation. Admin typography uses Plus Jakarta Sans throughout the shell, pages, forms, tables, and dialogs, with JetBrains Mono reserved for technical log content. The Admin-only scale is 24px page, 18px section/dialog, 15px card, 14px body copy, 13px form/table/navigation, 12px supporting, 11px eyebrow/table heading, and 24px metric text. Shell density follows MAW with a 56px utility bar, 80px brand area, centred 1600px content maximum, and 32px by 24px desktop content padding. Motion uses 200ms sidebar travel, 300ms navigation grouping, 240ms popovers, 260ms opening and 190ms closing for collapsible content, plus reduced-motion fallbacks. These tokens and styles are scoped to Admin and do not affect `app/`.
