...
The Admin shell owns shared `--admin-card-*` presentation tokens for radius, border, surface depth, hover depth, and inset surfaces. Dashboard is the canonical card reference: shared statistic cards keep text on the left and their soft semantic icon tile on the right. `StatCard`, `DataPanel`, toolbars, Dashboard panels, and established domain-card selectors consume the same tokens so Students, Classes, Finance, Attendance, Calendar, Audit/Application Logs, Settings, and moderation do not develop separate card languages. Admin typography uses Plus Jakarta Sans throughout the shell, pages, forms, tables, and dialogs, with JetBrains Mono reserved for technical log content. A final Admin-only typography layer defines the shared 28px page, 20px section/dialog, 16px card, 14px body/value, 13px label/navigation, 12px supporting, 11px eyebrow/table-heading, and 24–30px metric scale; narrow screens reduce only page and section titles to 24px and 18px. These tokens and fonts are scoped to Admin and do not affect `app/`.
