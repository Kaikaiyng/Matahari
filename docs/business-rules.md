...
- Active guardians receive in-app entry and exit notifications by default. Admin may change those two school-wide notification defaults. Card/device credentials are encrypted and face templates are not stored by Matahari.
...
- **Account Provisioning & Deletion Policy**:
  - Matahari uses institution-provisioned accounts managed by school administrators. Self-registration is disabled.
