...
- Standard fees are managed through fee templates before student-level overrides.
...
- Invoices and receipts are separate business objects.
...
- Audit logs record old values and new values for changed records.
...
erDiagram
    ...
    schools ||--o{ fee_templates : defines
    fee_templates ||--o{ fee_template_items : contains
    fee_items ||--o{ fee_template_items : used_by
    students ||--o{ student_fee_template_assignments : has
    fee_templates ||--o{ student_fee_template_assignments : assigned
...
- `fee_templates`
- `fee_template_items`
- `student_fee_template_assignments`
...
Defines application modules used by permissions.

| Column | Type | Notes |
| --- | --- | --- |
| id | bigint unsigned | Primary key |
| name | varchar(100) | e.g. `Payments` |
| slug | varchar(100) | e.g. `payments` |
...
| name | varchar(150) | e.g. `Record Payment` |
| slug | varchar(150) | e.g. `payment.create` |
| created_at | timestamp |  |
| updated_at | timestamp |  |
...
Defines reusable fee structures such as Nursery Fee or Primary Year 1 Fee.

| Column | Type | Notes |
| --- | --- | --- |
| id | bigint unsigned | Primary key |
| school_id | bigint unsigned |  |
| name | varchar(255) | e.g. `Primary Year 1 Fee` |
| code | varchar(50) | Optional |
...
### 6.3 `fee_template_items`

Stores the fee items inside a template.

| Column | Type | Notes |
| --- | --- | --- |
| id | bigint unsigned | Primary key |
| school_id | bigint unsigned |  |
| fee_template_id | bigint unsigned |  |
| fee_item_id | bigint unsigned |  |
| amount | decimal(10,2) | Template amount |
| sort_order | int unsigned |  |
| status | enum | `active`, `inactive` |
| created_at | timestamp |  |
| updated_at | timestamp |  |
...
- Unique: `(fee_template_id, fee_item_id)`

### 6.4 `student_fee_template_assignments`

Assigns a fee template to a student.
...
- `(school_id, fee_template_id)`

### 6.5 `student_fee_assignments`

Stores direct student fee overrides or exceptions. Standard recurring fees should normally come through `student_fee_template_assignments`.
...
### 6.7 `student_discount_assignments`
...
4. For each student:
   a. Load active fee template assignments effective for invoice_month.
   b. Load active direct fee overrides effective for invoice_month.
   c. Load active discount assignments effective for invoice_month.
   d. Build invoice item snapshots from template items, overrides, and discounts.
   e. Calculate subtotal, discount_total, grand_total.
   f. Generate invoice_no from invoice sequence.
   g. Create invoice header.
   h. Create invoice_items rows.
   i. Write audit log with old_values = null and new_values containing invoice summary.
...
9. Write audit log with old_values and new_values for payment and invoice balance changes.
```
...
10. Write audit log with old_values = null and new_values containing receipt summary.
```
...
5. `fee_items`, `fee_templates`, `fee_template_items`
6. `student_fee_template_assignments`, `student_fee_assignments`
7. `discount_items`, `student_discount_assignments`
8. `invoice_sequences`, `invoices`, `invoice_items`
9. `payments`, `payment_allocations`
10. `receipt_sequences`, `receipts`
11. `audit_logs`
