...
describe('apiRequest', () => {
  ...
  it('reuses an existing CSRF cookie without another bootstrap request', async () => {
    ...
    expect(new Headers(request.headers).get('X-XSRF-TOKEN')).toBe('existing-token')
  })

  it('refreshes the CSRF cookie once after a 419 response', async () => {
    document.cookie = 'XSRF-TOKEN=stale-token; path=/'
    const fetchSpy = vi.spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce(new Response(JSON.stringify({ message: 'CSRF token mismatch.' }), { status: 419 }))
      .mockImplementationOnce(async () => {
    ...
    expect(fetchSpy).toHaveBeenNthCalledWith(2, '/api/csrf-cookie', expect.any(Object))
    const retriedRequest = fetchSpy.mock.calls[2][1] as RequestInit
