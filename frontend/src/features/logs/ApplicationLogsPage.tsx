...
export function ApplicationLogsPage({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  return <section className="page-stack application-logs-page">
    ...
    <section className="log-engine-panel" aria-label="Log reader status"><header><span><Zap size={15} /> Log reader & source</span><b className={error ? 'offline-error' : ''}>{error ? 'Reader unavailable' : 'Reader active'}</b></header><div className="log-engine-grid"><LogFact label="Environment" value={environments.join(', ') || 'No entries'} /><LogFact label="Source files" value={sources.join(', ') || 'No entries'} /><LogFact label="Matching entries" value={String(meta.total)} /><LogFact label="Page size" value={String(meta.per_page)} /><LogFact label="Read window" value={meta.truncated ? 'Truncated' : 'Complete'} /></div></section>

    <div className="log-level-tabs" aria-label="Log level filters">{levels.map((item) => { const Icon = item.icon; const count = item.value ? meta.level_counts[item.value] : meta.total; return <button key={item.label} type="button" className={`${filters.level === item.value ? 'active' : ''} ${item.value ? logLevelClass(item.value) : ''}`} aria-pressed={filters.level === item.value} onClick={() => changeLevel(item.value)}><Icon size={14} /> {item.label}<span>{count}</span></button> })}</div>
