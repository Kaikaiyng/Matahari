...
function CaseDetail({ item, platformMode, currentUserId, decision, reasonCode, reason, restrictionScope, busy, onDecision, onReasonCode, onReason, onRestrictionScope, onSubmit, onRestrict, onAppeal }: DetailProps) {
  ...
  return <section className="ugc-detail" aria-label={`Case #${item.id} detail`}><header><div><p>{human(item.priority)} case #{item.id}</p><h2>{human(item.reason_code)}</h2>{'tenant_id' in item && <span>Tenant {item.tenant_id} · School {item.school_id}</span>}</div>{item.overdue && <b><Clock3 />Overdue</b>}</header><div className="ugc-evidence"><h3>Preserved evidence</h3><p>{evidence?.body ?? 'User-level report; no content body attached.'}</p>{item.target_snapshot.post?.media.map((media) => <div key={media.id}><AlertTriangle /><span><strong>{media.name ?? `Media #${media.id}`}</strong><small>{media.mime_type} · {media.status}</small></span></div>)}</div><div className="ugc-decision"><label>Decision<select aria-label="Decision" value={decision} onChange={(event) => onDecision(event.target.value as ModerationDecision)}><option value="">Select action</option><option value="no_violation">No violation</option><option value="approve">Approve</option><option value="reject">Reject</option><option value="hide">Hide</option><option value="warn">Warn</option>{!platformMode && <option value="escalate">Escalate to platform</option>}</select></label><label>Reason category<select aria-label="Reason category" value={reasonCode} onChange={(event) => onReasonCode(event.target.value)}><option value="">Select reason</option>{reasons.map((value) => <option key={value} value={value}>{human(value)}</option>)}</select></label><label>Decision reason<textarea aria-label="Decision reason" rows={4} value={reason} onChange={(event) => onReason(event.target.value)} /></label><button type="button" className="ugc-primary" disabled={busy || !decision || !reasonCode || !reason.trim()} onClick={() => void onSubmit()}>Apply decision</button>{!platformMode && <div className="ugc-restrict"><label>Community restriction<select value={restrictionScope} onChange={(event) => onRestrictionScope(event.target.value as DetailProps['restrictionScope'])}><option value="comment">Comments only</option><option value="publish">Publishing only</option><option value="media">Media only</option><option value="all">All Community contribution</option></select></label><button type="button" disabled={busy || !reasonCode || !reason.trim()} onClick={() => void onRestrict()}>Apply restriction</button></div>}</div>{!platformMode && item.appeals?.map((appeal) => <article className="ugc-appeal" key={appeal.id}><h3>Appeal #{appeal.id}</h3><p>{appeal.statement}</p>{appeal.source_moderator_user_id === currentUserId ? <strong>A different moderator must decide this appeal.</strong> : appeal.status === 'submitted' ? <div><button type="button" disabled={busy} onClick={() => void onAppeal(appeal.id, 'upheld')}>Uphold appeal</button><button type="button" disabled={busy} onClick={() => void onAppeal(appeal.id, 'overturned')}>Overturn decision</button></div> : <span>{human(appeal.decision ?? appeal.status)}</span>}</article>)}</section>
}
